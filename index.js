...
function showSlides() {
  ...
  let eventDate = new Date( yearOfTheEvent , 10, 14 )
  const isItFebruary4th = currentTime.getMonth() === 10 && currentTime.getDate() === 14

  function countdown() {
    const now = new Date()
    
    if (now > eventDate) {
      eventDate = new Date( yearOfTheEvent + 1, 10, 14 )
    } else if ( now.getFullYear() === eventDate.getFullYear() + 1 ) {
      eventDate = new Date( now.getFullYear(), 10, 14 )
    }
    ...
    if ( isItFebruary4th ) {
      console.log('Happy birthday, Jagriti!')
